...
public class MPToJSON {
	...
	private JsonObject constraintToJson(Constraint c){
		...
		.add("ComparisonOperator", comparaisonOpToString(c.getOperator()))
		.add("Rhs", c.getRhs())
	...
	private String comparaisonOpToString(ComparisonOperator op){
	 ...
	 private JsonObject termToJson(Term a){
		 JsonObjectBuilder ob=Json.createObjectBuilder();
		 ob.add("Coefficient", a.getCoefficient())
		 .add("Variable", variableToJson(a.getVariable()));
		 return ob.build();
	 }
	 ...
	 private JsonObject variableToJson(Variable x){
		 JsonObject model=Json.createObjectBuilder()
				 ...
				 .add("Range", rangeToJson(x.getBounds()))
				 .build();
		 return model;
	 }
