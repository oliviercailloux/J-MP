...
public class MPToJSON {
	...
	public JsonObject getJson(IMP mp) {
		...
		JsonObject model = Json.createObjectBuilder().add("ProblemName", mp.getName())
				.add("Objective", objectiveToJson(mp.getObjective()))
				.add("Constraints", constrantsToJson(mp.getConstraints())).build();
	...
	private JsonObject constraintToJson(Constraint c) {
		...
		ob.add("Lhs", sumTermsToJson(c.getLhs())).add("ComparisonOperator", comparaisonOpToString(c.getOperator()))
	...
	private JsonObject termToJson(Term a) {
		JsonObjectBuilder ob = Json.createObjectBuilder();
		ob.add("Coefficient", a.getCoefficient()).add("Variable", variableToJson(a.getVariable()));
		return ob.build();
	}

	private JsonObject variableToJson(Variable x) {
		JsonObject model = Json.createObjectBuilder().add("Name", x.getName())
				.add("VariableDomain", x.getDomain().toString()).add("Range", rangeToJson(x.getBounds())).build();
		return model;
	}
